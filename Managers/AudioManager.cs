...
namespace CustomAlbums.Managers
{
    public static class AudioManager
    {
        ...
        private static readonly Utilities.Logger Logger = new(nameof(AudioManager));
        ...
        public static AudioClip LoadClipFromOgg(Stream stream, string name)
        {
            ...
            coroutine = CreateCoroutine((Il2CppSystem.Func<bool>)delegate
            {
                ...
                catch (Exception e)
                {
                    ...
                    {
                        Logger.Warning("Possible over-read of audio file. This is a file-dependent anomaly. Consider making a minimal edit and re-saving.");
                    }
                    Logger.Error($"Exception while reading at offset {index} of {((double)ogg.Position / 1000) / ogg.Length} in {name}, aborting: {e.Message}");
        ...
        private static Coroutine CreateCoroutine(Il2CppSystem.Func<bool> update)
        {
            return SingletonMonoBehaviour<CoroutineManager>.instance.StartCoroutine(
                (Il2CppSystem.Action)delegate { }, update);
        }
